...
class MultiObjectiveInfill(BaseInfill):
    ...
    Attributes:
        ...
        obj_indices (List[int]): Output indices minimised as objectives.
        ...
        S_nd (np.ndarray): Non-dominated IS samples in normalised obj space. shape: (N_nd, M).
    ...
    def __init__(
        ...
        Args:
            ...
            obj_indices (List[int]): Output indices treated as objectives (minimised).
            constraint_indices (Optional[List[int]]): Output indices for constraints.
        ...
        """
        super().__init__(model, bounds=bounds, target_index=0, num_restarts=num_restarts)

        self.obj_indices = list(obj_indices)
        self.constraint_indices = list(constraint_indices) if constraint_indices else None
        ...
        if constraint_ubs is not None:
            ...
            if (self.constraint_indices is not None
                    and len(self.constraint_ubs) != len(self.constraint_indices)):
                raise ValueError(
        ...
        y_obj = np.array(y_train[:, self.obj_indices], dtype=np.float64)
    ...
    def _precompute_samples(self, y_obj: np.ndarray) -> None:
        ...
        Steps:
        1. Draw N_E uniform samples S in [obj_lb, obj_ub]^M (normalised space).
        ...
        for start in range(0, N_nd, chunk_size):
            ...
            batch = self.S_nd[start:end]
            b_exp = batch[:, np.newaxis, :]
            s_exp = self.S_nd[np.newaxis, :, :]
            diff  = s_exp - b_exp
    ...
    def _compute_ehvi_batch(self, x_candidates: np.ndarray) -> np.ndarray:
        ...
        Returns:
            np.ndarray: cEHVI values per candidate. shape: (N_c, 1).
        """
        ...
        mu_obj   = mu_raw[:, self.obj_indices]
        var_obj  = var_raw[:, self.obj_indices]
        ...
        ehvi           = (pdf_f @ self.hvi_nd) / self.N_E
        ehvi[zero_var] = 0.0

        if self.constraint_indices is not None and self.constraint_ubs is not None:
            mu_c    = mu_raw[:, self.constraint_indices]
            var_c   = var_raw[:, self.constraint_indices]
    ...
    def _compute_diversity_batch(self, mu_obj_norm: np.ndarray) -> np.ndarray:
        ...
        Args:
            mu_obj_norm (np.ndarray): Normalised predicted objectives. shape: (N_c, M).
    ...
    def propose(self) -> np.ndarray:
        ...
        top_indices    = np.argsort(ehvi[:, 0])[::-1][:n_top]
        P_w            = x_candidates[top_indices]

        mu_raw_pw, _  = self.model.predict(P_w)
        mu_obj_pw     = mu_raw_pw[:, self.obj_indices]
